...
int LeerPregunta(int numOperacion, int *num1, char *operando, int *num2, int *es_operacion_continua, int resultado_anterior) {
  ...
  LimpiarInput(input, num1, operando, num2, &resultado_anterior);
...
int main() {
    ...
    while (1) {
        int input = LeerPregunta(numOperacion, &num1, &operando, &num2, &es_operacion_continua, resultado_anterior);
        if (input == 0) {
